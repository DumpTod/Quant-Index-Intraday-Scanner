# data_fetcher.py — Fyers API candle data using SDK (correct method)
...
def _get_fyers() -> fyersModel.FyersModel:
    """Build authenticated FyersModel — token format: client_id:access_token"""
...
def fetch_15min_candles(index: str, from_date: date = None, to_date: date = None) -> pd.DataFrame:
    """Fetch 15-min OHLCV candles. Defaults to today."""
    fyers  = _get_fyers()
    ...
    data = {
        ...
        "date_format": "0",          # 0 = epoch timestamps
        "range_from":  _date_str(from_d),
        "range_to":    _date_str(to_d),
        "cont_flag":   "1",
    }
...
def fetch_daily_candles(index: str, n_days: int = 5) -> pd.DataFrame:
    """Fetch last n_days daily candles — for CPR calculation."""
    fyers  = _get_fyers()
    symbol = INSTRUMENTS[index]
    today  = date.today()
...
def fetch_candles_after(index: str, signal_dt: datetime, to_date: date = None) -> pd.DataFrame:
    """Fetch 15-min candles after signal_dt — for history rescan."""
