...
# Import maps from config (covers NIFTY, BANKNIFTY, SENSEX)
from config import LOT_SIZES_MAP as LOT_SIZES, STRIKE_INTERVALS_MAP as STRIKE_INTERVALS
...
def suggest_options(index: str, direction: str, ltp: float) -> dict:
    ...
    return {
        ...
        "atm_strike":   atm,
        "itm_strike":   itm,
        "atm_symbol":   f"{exchange}:{index}26MAR{atm}{opt_type}",   # update monthly
        "itm_symbol":   f"{exchange}:{index}26MAR{itm}{opt_type}",
