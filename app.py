...
def debug_fyers(index: str):
    ...
    try:
        ...
        import requests as req
        params = {"symbol": INSTRUMENTS[index], "resolution": "15", "date_format": 1,
                  "range_from": today.strftime("%Y-%m-%d"), "range_to": today.strftime("%Y-%m-%d"), "cont_flag": 1}
        headers = {"Authorization": f"{FYERS_CLIENT_ID}:{tok}", "Content-Type": "application/json"}
        r    = req.get("https://api-t1.fyers.in/data/history", headers=headers, params=params, timeout=30)
        resp = r.json()
